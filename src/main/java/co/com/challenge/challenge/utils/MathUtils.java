...
public class MathUtils {
  ...
  public static  LineEquation addEquations(final LineEquation equationOne, final LineEquation equationTwo) {
    ...
    //sumEquations(equationOne,equationTwo);
  ...
  public static  LineEquation addEquations(
      final LineEquation equationOne, final LineEquation equationTwo,final LineEquation equationThree) {

    final float A = 2 * equationTwo.getComponentX() - 2 * equationOne.getComponentX();
    final float B = 2 * equationTwo.getComponentY() - 2 * equationOne.getComponentY();
    final float C = floatPow(29F,POWER_TWO) -
        floatPow(28F,POWER_TWO) -
        floatPow(equationOne.getComponentX(),POWER_TWO) +
        floatPow(equationTwo.getComponentX(),POWER_TWO) -
        floatPow(equationOne.getComponentY(),POWER_TWO) +
        floatPow(equationTwo.getComponentY(),POWER_TWO);
    final float D = (2 *  equationThree.getComponentX()) - (2 * equationTwo.getComponentX());
    final float E = (2 * equationThree.getComponentY()) - (2 * equationTwo.getComponentY());
    final float F = floatPow(28F,POWER_TWO) -
        floatPow(52F,POWER_TWO) -
            floatPow(equationTwo.getComponentX(),POWER_TWO) +
        floatPow(equationThree.getComponentX(),POWER_TWO) -
        floatPow(equationTwo.getComponentY(),POWER_TWO) +
        floatPow(equationThree.getComponentY(),POWER_TWO);
    final float x = (C*E - F*B) / (E*A - B*D);
    final float y = (C*D - A*F) / (B*D - A*E);

    final PointModel kenobi = PointModel.builder()
        .componentX(1)
        .componentY(3)
        .build();

    final PointModel possiblePoint = PointModel.builder()
        . componentX(x)
        .componentY(y)
        .build();

    final float d = distanceBetweenTwoPoints(kenobi,possiblePoint);

    return null;
  }

  public static void sumEquations(final LineEquation equationOne, final LineEquation equationTwo) {
    final float A = equationOne.getComponentX();
    final float B = equationOne.getComponentY();
    final float C = equationOne.getOrdered();
    final float D = equationTwo.getComponentX();
    final float E = equationTwo.getComponentY();
    final float F = equationTwo.getOrdered();
    ...
    distanceBetweenTwoPoints(kenobi,possiblePoint);
